...
### Added
- Pathfinder fork: bearer-authed `GET /images/:storage_id.png` route on the OAuth sidecar for local downloads.
- `download_urls[]` field in `generate_image` `structured_content` (requires `NANOBANANA_PUBLIC_URL` env).
- `NANOBANANA_PUBLIC_URL` env var.
